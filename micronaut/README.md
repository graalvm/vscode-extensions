...
When installed, the extension might check whether there is a registered GraalVM installation, and eventually request to download it or point to a local installation (see [GraalVM Installation and Setup in VS Code](https://marketplace.visualstudio.com/items?itemName=oracle-labs-graalvm.graalvm#installation-and-setup)).
...
[Having set GraalVM as the default runtime and debug environment](https://marketplace.visualstudio.com/items?itemName=oracle-labs-graalvm.graalvm#installation-and-setup) in VS Code, invoke the **View > Command Palette > Micronaut: Build...** action, where you can select the build targets (e.g., `clean`, `build`, `nativeImage`, etc.) from a list of available ones.
...
If installed in VSCode Oracle Developer Tools for VS Code (SQL and PLSQL) extension can be used to register OCI database for Micronaut project. This allows Micronaut data support to use connected database objects in developing Micronaut applications
